...
export default function CourseAdminPage() {
  ...
  const handleConfirmDelete = async () => {
    try {
      await dispatch(deleteCourse(deletePopup.id)).unwrap();
  ...
  const handleEditClick = (course) => {
    setEditModal({ show: true, course: course });
  };
  ...
  const getCategoryName = (categoryId) => {
    const category = categoryData.find(cat => cat.courseCategoryId === categoryId);
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
        <div className='md:mb-2 mb-4'>
          <h1 className="text-xl md:text-2xl font-bold text-[#2E4A5B] dark:text-white">Course Management</h1>
          <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mt-0.5 md:mt-1">Manage your courses and content</p>
        </div>
      </div>
      ...
      <div id="course-table-container" className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        {/* Table View */}
        {viewMode === 'table' ? (
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right">Actions</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-16">Course Video</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-16">Thumbnail</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-16">Title</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Description</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Category</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rating</th>
                  <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">No. of Ratings</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {currentCourses.length > 0 ? (
                  currentCourses.map((course, index) => {
                    ...
                    return (
                      <tr key={courseData.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Course Video">
                            ...
                            <div className="min-w-0 max-w-[200px]">
                              {courseData.video ? (
                                ...
                                <div className="w-full h-24 bg-gray-200 dark:bg-gray-700 rounded flex items-center justify-center">
                                  <RiVideoLine className="text-gray-400 text-2xl" />
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Thumbnail">
                        ...
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Title">
                          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300 truncate block max-w-[200px]" title={courseData.title}>
                            {courseData.title}
                          </span>
                        </td>
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Description">
                          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300 truncate block max-w-[200px]" title={courseData.description}>
                            {courseData.description}
                          </span>
                        </td>
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Category">
                          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300">
                            {courseData.categoryName}
                          </span>
                        </td>
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Rating">
                          <div className="flex items-center gap-1">
                            <RiStarLine className="text-amber-500 text-sm" />
                            <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300">
                              {parseFloat(courseData.rating).toFixed(1)}
                            </span>
                          </div>
                        </td>
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="No. of Ratings">
                          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300">
                            {courseData.noOfRating}
                          </span>
                        </td>
                        <td className="px-3 md:px-4 py-3 md:py-4" data-label="Status">
                          <span className={`inline-flex px-2 py-1 rounded-full text-xs justify-center font-medium ${
                            courseData.status === 1 || courseData.status === 'Active' 
                              ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                              : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                          }`}>
                            {courseData.status === 1 || courseData.status === 'Active'  ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-8 md:py-12 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">No courses found</p>
                      </div>
                    </td>
                  </tr>
                )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
              {currentCourses.length > 0 ? (
                currentCourses.map((course, index) => {
                  ...
                  return (
                    <div key={courseData.id} className="bg-gray-50 dark:bg-gray-700/30 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      <div className="relative h-32 md:h-40">
                        <Image
                          ...
                          onError={(e) => {
                            ...
                            e.target.src = '/assets/img/blog/blog-img-1-1.jpg';
                          }}
                        />
                        ...
                        <div className="absolute top-2 right-2">
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ${courseData.status === 1 || courseData.status === 'published'
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-3">
                          <div className="flex items-center gap-1">
                            ...
                            <span>{parseFloat(courseData.rating).toFixed(1)}</span>
                          </div>
                          <span>{courseData.noOfRating} ratings</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => handleEditClick(course)}
                          ...
                          <button
                            onClick={() => handleDeleteClick(courseData.courseId)}
        ...
        {filteredCourses.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 md:p-4 border-t border-gray-100 dark:border-gray-700">
            <div className="text-xs md:text-sm text-gray--400 text-center500 dark:text-gray sm:text-left order-2 sm:order-1">
              Showing <span className="font-medium">{indexOfFirstItem + 1}</span> to <span className="font-medium">{Math.min(indexOfLastItem, filteredCourses.length)}</span> of <span className="font-medium">{filteredCourses.length}</span> entries
            </div>
...
function EditCourseModal({ course, onClose, onUpdate, isEditing, categoryData }) {
  ...
  const getCourseField = (field, fallback = '') => {
    ...
    if (field === 'thumbnail') return course.thumbnailImage || course.thumbnail || course.ThumbnailImage || course.Thumbnail || null;
    if (field === 'video') return course.courseVideo || course.video || course.CourseVideo || course.Video || null;
    if (field === 'courseId') return course.courseId || course.id || course.CourseId || '';
    if (field === 'categoryId') return course.categoryId || course.CategoryId || course.courseCategoryId || '';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4 md:p-6 flex items-center justify-between">
          <h2 className="text-xl font-bold text-[#2E4A5B] dark:text-white">Edit Course</h2>
          <button
            onClick={onClose}
              ...
              <div className="flex flex-col items-start gap-3">
                <div className="w-full h-40 relative rounded-lg overflow-hidden border-2 border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                  {videoPreview ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <RiVideoLine className="text-3xl text-gray-400" />
                    </div>
                  )}
                </div>
                <input
