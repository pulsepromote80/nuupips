...
const News = () => {
    ...
    const filteredNews = news?.filter(course =>
            ...
            <div className="space-y-4 md:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
                    <div className='md:mb-2 mb-4'>
                        <h1 className="text-xl md:text-2xl font-bold text-[#2E4A5B] dark:text-white">News Management</h1>
                        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mt-0.5 md:mt-1">Manage your news and content</p>
                    </div>
                </div>
                        ...
                        <div className="overflow-x-auto w-full">
                            <table className="w-full text-left border-collapse min-w-[900px]">
                                <thead className="bg-gray-50 dark:bg-gray-700/50">
                                    <tr>
                                        <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center">Actions</th>
                                        <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-16">Images</th>
                                        <th className="px-4 py-3 text-[10px] md:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-16">Name</th>
                                            ...
                                            return (
                                                <tr key={courseData.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                                    ...
                                                    <td className="px-3 md:px-4 py-3 md:py-4" data-label="Status">
                                                        <span className={`inline-flex px-2 py-1 rounded-full text-xs justify-center font-medium ${courseData.status === 1 || courseData.status === 'Active'
                                ...
                                {currentNews.length > 0 ? (
                                    currentNews.map((course, index) => {
                                        ...
                                        const courseData = {
                                            ...
                                            title: course.title || course.Title || course.tittle || '-',
                                            description: course.description || course.Description || '-',
                                            image: course.image || '/assets/img/blog/blog-img-1-1.jpg',
                                            video: course.courseVideo || course.video || course.CourseVideo || course.Video || '',
                                            CourseCategoryId: course.categoryId || course.CategoryId || course.courseCategoryId || '',
                                            categoryName: course.categoryName || course.CategoryName || course.courseCategoryName || getCategoryName(course.categoryId) || '-',
                                            rating: course.rating || course.Rating || 0,
                    ...
                    {filteredNews?.length > 0 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 md:p-4 border-t border-gray-100 dark:border-gray-700">
                            <div className="text-xs md:text-sm text-gray--400 text-center500 dark:text-gray sm:text-left order-2 sm:order-1">
                                Showing <span className="font-medium">{indexOfFirstItem + 1}</span> to <span className="font-medium">{Math.min(indexOfLastItem, filteredNews.length)}</span> of <span className="font-medium">{filteredNews.length}</span> entries
                            </div>
